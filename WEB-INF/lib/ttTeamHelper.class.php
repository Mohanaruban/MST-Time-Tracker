...
static function getManagerListReport() {
  ...
  static function getActiveUsers($options = null) {
    ...
    if (isset($options['getAllFields']))
      $sql = "select * from tt_users where team_id = $user->team_id and status = 1 and role != 1024 order by name";
    else
      $sql = "select id, name from tt_users where team_id = $user->team_id and status = 1 role != 1024 order by name";
    $res = $mdb2->query($sql);
    $user_list = array();
    if (is_a($res, 'PEAR_Error'))
      return false;
  ...
  static function getActiveProjectView($options = null) {
    ...
    if (isset($options['getAllFields']))
      $sql = "select u.name,u.login,u.role, GROUP_CONCAT(p.name SEPARATOR '<br>') as projects from tt_users u inner join tt_user_project_binds upb on u.id = upb.user_id inner join tt_projects p on upb.project_id = p.id where u.status = 1 and u.role != 1024 and upb.status = 1 group by u.name order by u.name";
    else
      $sql = "select u.name,u.login,u.role, GROUP_CONCAT(p.name SEPARATOR '<br>') as projects from tt_users u inner join tt_user_project_binds upb on u.id = upb.user_id inner join tt_projects p on upb.project_id = p.id where u.status = 1 and u.role != 1024 and upb.status = 1 group by u.name order by u.name";
...
// The getActiveUsers obtains all active users in a given team.
  static function getActiveUsersAdmin($options = null) {
    ...
    if (isset($options['getAllFields']))
      $sql = "select * from tt_users where status = 1 and role != 1024 order by name";
    else
      $sql = "select id, name from tt_users where status = 1 and role != 1024 order by name";
    $res = $mdb2->query($sql);
    $user_list = array();
    if (is_a($res, 'PEAR_Error'))
      return false;
